...
def return_fake_perform_post_request(*args, **kwargs):
    if kwargs['url'] == 'core/api/otl/assets/search' and kwargs['json_data']['fromCursor'] is None:
        response = Mock(Response)
        response.content = json.dumps(EMInfraAPIMock.assets_first_page).encode('utf-8')
        response.headers = {'em-paging-next-cursor': 'cursor1'}
        return response
    elif kwargs['url'] == 'core/api/otl/assets/search' and kwargs['json_data']['fromCursor'] == 'cursor1':
        response = Mock(Response)
        response.content = json.dumps(EMInfraAPIMock.assets_second_page).encode('utf-8')
...
def test_fill_then_fix_tz():
    fill_manager = create_fill_manager(ResourceEnum.assets)
    ...
    select_q = """SELECT ?asset WHERE { GRAPH ?g { ?asset a <https://lgc.data.wegenenverkeer.be/ns/installatie#Kast> } }"""
